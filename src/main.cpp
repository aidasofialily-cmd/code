...
int main() {
    ...
    while (true) {
        // BOLT OPTIMIZATION: Use blocking poll with calculated timeout instead of busy-wait sleep.
        // This reduces latency from ~10ms to near 0ms and eliminates unnecessary CPU wakeups.
        int timeout_val = hardMode ? 100 : 1000;
        auto now = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_tick).count();
        int wait_time = std::max(0, timeout_val - (int)elapsed);
