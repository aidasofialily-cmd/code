...
int main() {
    ...
    while (true) {
        ...
        if (updateUI) {
            std::cout << "\r" << CLR_EOL << CLR_SCORE << "Score: " << formatWithCommas(score) << CLR_RESET << " | High: " << formatWithCommas(highscore) << " "
    ...
    if (score > initialHighscore) {
        std::cout << CLR_NORM << "Congratulations! A new personal best (was " << formatWithCommas(initialHighscore) << ")!" << CLR_RESET << "\n";
    }
