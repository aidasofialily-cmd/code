...
## 2026-05-24 - Eliminating Trailing Text Artifacts in CLI
**Learning:** When using carriage return `\r` to repeatedly overwrite a line in the terminal, relying on hardcoded spaces for padding can fail or leave artifacts if the new text is shorter than the old.
**Action:** Always use the ANSI "Erase in Line" sequence `\033[K` immediately after the carriage return `\r\033[K` to dynamically clear the line before printing new content, ensuring a clean update without artifacts.
