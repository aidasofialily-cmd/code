...
## 2024-05-27 - Preventing CLI Trailing Text Artifacts
**Learning:** Hardcoding padding spaces to clear previous text when using `\r` for dynamic CLI line updates is brittle and can leave artifacts. The ANSI escape sequence `\033[K` (Erase in Line) is the proper UX solution for clearing the rest of the line.
**Action:** Always use `\033[K` immediately after a carriage return (`\r`) when updating terminal lines in place.
