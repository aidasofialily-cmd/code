...
## 2025-01-24 - Robust Line Clearing in CLI
**Learning:** Manual space padding to clear previous characters in volatile carriage-returned (\r) terminal lines is fragile and prone to leaving visual artifacts if the new string is shorter than expected. Using the ANSI escape sequence \033[K (Erase in Line) provides a clean, professional way to ensure the remainder of the line is cleared regardless of length changes.
**Action:** Prefer \033[K (CLR_EOL) over manual space padding for in-place UI updates in terminal applications.
