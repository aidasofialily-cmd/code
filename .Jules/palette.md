...
## 2024-05-27 - Empty States in CLI
**Learning:** In interactive CLI applications, providing explicit, encouraging empty states for data or achievements rather than hiding the UI element when empty clarifies system state and improves user onboarding.
**Action:** When implementing UX in CLI applications, explicitly provide an empty state (e.g., "None yet! Set a record!") instead of hiding the achievement display.
