...
## 2025-01-25 - CLI Readability and Visual Polish
**Learning:** In text-heavy CLI interfaces, large numbers become hard to read at a glance. Implementing thousands separators significantly improves readability. Additionally, using the CLR_EOL (\033[K) sequence when updating lines with \r is essential to prevent "ghost characters" when a new string is shorter than the previous one.
**Action:** Always format large numeric values with commas and use CLR_EOL for clean, flicker-free terminal UI updates.
