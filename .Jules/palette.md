...
## 2024-05-22 - Explicit Empty States in CLI
**Learning:** When implementing UX in CLI applications, provide explicit, encouraging empty states for data or achievements rather than hiding the UI element when empty. This clarifies system state and improves user onboarding.
**Action:** Added an explicit empty state for the personal best highscore display, providing a clear call-to-action when there is no record yet.
