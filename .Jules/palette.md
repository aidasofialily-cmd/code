...
## 2024-05-24 - Avoiding Terminal Artifacts
**Learning:** When dynamically updating terminal lines via carriage return (\r), relying on hardcoded trailing spaces to overwrite previous output is error-prone and can leave artifacts. Using the ANSI "Erase in Line" escape sequence (\033[K) is more robust.
**Action:** Use \033[K after \r when updating dynamic terminal output to ensure clean lines without trailing characters.
