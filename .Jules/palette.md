...
## 2026-05-24 - Clean Line Updates in CLI
**Learning:** Using trailing spaces to "clear" old text in carriage-returned (`\r`) lines is fragile and often leaves "ghost" characters if the new string is shorter than the previous one. The ANSI escape sequence `\033[K` (Erase in Line) is the robust way to ensure a clean UI by clearing everything from the cursor to the end of the line.
**Action:** Always append `\033[K` to the end of lines that use `\r` for in-place updates in terminal applications.
