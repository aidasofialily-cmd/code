...
## 2026-05-31 - Robust CLI HUD Updates
**Learning:** Utilizing the ANSI 'Erase in Line' escape sequence (`\033[K`) is a more robust UX practice than space padding for in-place terminal updates, as it guarantees the removal of all characters from the previous line regardless of length. Additionally, using thousands separators for numeric displays in CLI tools significantly improves readability as values escalate.
**Action:** Use the `CLR_EOL` macro (targeting `\033[K`) and implement a `formatWithCommas` helper for all multi-digit numeric displays in CLI interfaces.
