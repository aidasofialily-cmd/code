...
## 2024-05-24 - Add Empty State for CLI Highscore
**Learning:** CLI applications often hide UI elements like scores when empty, leading to an unclear system state. Providing an explicit, encouraging empty state clarifies the state and improves user onboarding.
**Action:** Always provide explicit empty states for data or achievements rather than hiding the element when empty.
