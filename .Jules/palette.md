...
## 2024-05-23 - Score Readability and Clean CLI Transitions
**Learning:** Large numeric values in terminal interfaces become difficult to parse at a glance as they grow. Thousands separators significantly reduce cognitive load. Additionally, using carriage returns (\r) for in-place updates often leaves "ghost characters" from previous longer strings. The ANSI Erase in Line sequence (\033[K) is essential for clean transitions.
**Action:** Implement a standard `formatWithCommas` utility for all numeric displays and define a `CLR_EOL` macro to clear the remainder of the line during every `\r` update.
